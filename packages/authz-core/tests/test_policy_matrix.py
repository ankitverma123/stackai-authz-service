...
def test_every_action_has_at_least_one_permit() -> None:
    ...
    permits = [
        block for block in re.split(r"@id\(", policies)
    ...
    assert not ungranted, (
        f"actions with no permit anywhere — Cedar default-deny makes these "
        f"unreachable: {ungranted}"
    )
...
def test_exported_workflow_access(
    engine: PolicyEngine,
    exported: bool, visibility: str, protected: bool,
    verified: bool, in_org: bool, expected: bool, why: str,
) -> None:
    """Extras #3 and #4 compose without ordering logic because both are forbids."""
    principal, slice_ = build_slice(
        "anonymous", exported=exported, visibility=visibility,
        password_protected=protected, in_org=in_org,
    )
    decision = engine.authorize(
        principal=principal,
...
def test_same_super_admin_via_jwt_can_delete_team(engine: PolicyEngine) -> None:
    ...
    principal, slice_ = build_slice("super_admin")
    decision = engine.authorize(
        principal=principal, action=Action.TEAM_DELETE, resource=TEAM,
        slice_=slice_, context=AuthzContext(auth_method="jwt"),
    )
